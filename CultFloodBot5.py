...
class Database:
    ...
    def _init_db(self):
        with sqlite3.connect(DB_PATH) as conn:
            ...
            # 🔥 АВТО-МИГРАЦИЯ: добавляем отсутствующие колонки в старую базу данных
            migrations = [
...
def get_application_actions_keyboard(app_id):
    kb = types.InlineKeyboardMarkup()
    # Две кнопки одобрения и одна отклонения
    kb.row(
...
def admin_manage_callbacks(call):
    ...
    if call.data == "add_admin":
        ...
        for admin_id, lvl, added_at, expires_at in admins:
            ...
            text += f"🆔 `{admin_id}` | @{clean_html(username)}\n📊 Уровень: {lvl_name} ({time_text})\n\n"
...
# 🔥 КНОПКА ОДОБРЕНИЯ С ОБЫЧНОЙ ССЫЛКОЙ
...
# 🔥 НОВАЯ КНОПКА: ОДОБРИТЬ С ОДНОРАЗОВОЙ ССЫЛКОЙ
...
def approve_app_with_one_time_link(call):
    try:
        ...
        # Создаем одноразовую ссылку (member_limit=1 означает ровно 1 использование)
        try:
...
# 🔥 КНОПКА ОТКЛОНЕНИЯ И СТЕЙТ ПРИЧИНЫ
...
# ========== НАВИГАЦИЯ И ОТМЕНА ==========
@bot.callback_query_handler(func=lambda call: call.data.data == "back_to_admin" if hasattr(call.data, "data") else call.data == "back_to_admin")
