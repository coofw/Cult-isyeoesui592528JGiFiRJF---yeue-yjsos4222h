...
class Database:
    ...
    def create_admin_code(self, code, level, duration, max_uses, created_by):
        with get_db_connection() as conn:
            c = conn.cursor()
            # Сбрасываем всю предыдущую историю этого кода
    ...
    def use_admin_code(self, code, user_id, allow_repeat=False):
        ...
        # Выполняем активацию вне контекстного менеджера для исключения блокировок БД
...
# ========== СКАЧАТЬ БД ПО КНОПКЕ (LEVEL 4 + MASTER ADMIN) ==========
...
def show_admin_stats(call):
    ...
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("🔙 Изменить период", callback_data=f"stat_adm_{admin_id}"))
...
# ========== КОД (АКТИВАЦИЯ И ПОВТОРНЫЙ ВВОД) ==========
...
# ----- ОБРАБОТКА ПОТВЕРЖДЕНИЯ ПОВТОРНОГО ВВОДА -----
