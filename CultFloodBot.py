...
CHAT_ID = -1003975292023  # TODO: Укажи здесь ID твоего чата/группы (начинается с -100)
CHAT_INVITE_LINK = "https://t.me/+rqod3GyElkwxYzYy"  # Резервная ссылка, если бот не админ в чате
...
@bot.message_handler(state=AddAdminState.waiting_for_level, chat_types=['private'])
...
@bot.message_handler(state=CreateCodeState.waiting_for_level, chat_types=['private'])
...
def approve_app(call):
    try:
        ...
        # Генерация одноразовой пригласительной ссылки
        try:
            ...
            logger.error(f"Ошибка при создании одноразовой ссылки: {e}")
            invite_link = CHAT_INVITE_LINK  # Фоллбэк на дефолтную ссылку

        # Обновление всех сообщений-уведомлений у всех админов
        ...
        for admin_id, msg_id in notifications:
            try:
                ...
                logger.error(f"Не удалось обновить карточку анкеты у админа {admin_id}: {e}")
...
def reject_app_process(message):
    ...
    try:
        ...
        if app:
            ...
            # Обновление всех уведомлений об этой анкете у всех админов
            notifications = db.get_notifications("application", app_id)
            updated_text = (
            ...
            for notif_admin_id, notif_msg_id in notifications:
                try:
                    ...
                    logger.error(f"Ошибка редактирования карточки анкеты у админа {notif_admin_id}: {e}")
...
def close_ticket(call):
    ...
    # Обновляем статус во всех уведомлениях об этом тикете у всех админов
    ...
    for admin_id, msg_id in notifications:
        try:
            ...
            logger.error(f"Не удалось обновить статус тикета у админа {admin_id}: {e}")
