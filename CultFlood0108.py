...
MASTER_ADMIN_IDS = [8484944484]  # Твой ID (владелец - 4 уровень)
...
class Database:
    ...
    def _init_db(self):
        with sqlite3.connect(DB_PATH) as conn:
            ...
            # Инициализация Главного Владельца (Уровень 4)
    ...
    def use_admin_code(self, code, user_id):
        with sqlite3.connect(DB_PATH) as conn:
            ...
            if current_level == 0 or code_level > current_level:
                ...
                conn.commit()
                return True, f"✅ Поздравляем! Вы получили уровень: {LEVEL_NAMES.get(code_level, 'Админ')}!"
            else:
    ...
    def get_ticket(self, ticket_id):
        with sqlite3.connect(DB_PATH) as conn:
            c = conn.cursor()
            c.execute("SELECT id, user_id, status, created_at, closed_by, closed_at FROM tickets WHERE id = ?", (ticket_id,))
            return c.fetchone()
    ...
    def get_archived_tickets(self, limit=5, offset=0):
        with sqlite3.connect(DB_PATH) as conn:
            c = conn.cursor()
            c.execute("SELECT id, user_id, status, created_at, closed_by FROM tickets ORDER BY created_at DESC LIMIT ? OFFSET ?", (limit, offset))
            rows = c.fetchall()
...
def cmd_start(message):
    ...
    if db.is_banned(user_id):
        bot.send_message(message.chat.id, "🚫 **Вы в чёрном списке!** Обратитесь к администрации.")
        return
...
def start_ticket(message):
    ...
    if not open_tickets:
        ...
        bot.send_message(message.chat.id, "✏️ **Напиши своё сообщение для админов:**\n\n❌ Отмена — /cancel")
    else:
        kb = types.InlineKeyboardMarkup()
...
def process_ticket(message):
    ...
    admins = db.get_all_admins()
    for admin_id, level, _, _ in admins:
        if level >= 2: # Тикеты получают только 2+ ур
            try:
                msg = bot.send_message(
                    admin_id,
...
def admin_panel(message):
    ...
    if level < 1:
        ...
        return
    bot.send_message(message.chat.id, "👑 **Админ панель**", reply_markup=get_admin_keyboard(level))
...
def admin_tickets(message):
    if db.get_admin_level(message.from_user.id) < 2:
        bot.send_message(message.chat.id, "❌ У вас нет доступа к просмотру открытых тикетов (нужен уровень 2+).")
        return
...
def approve_app(call):
    try:
        ...
        # СОХРАНЯЕМ ТЕКСТ АНКЕТЫ В УВЕДОМЛЕНИИ!
...
def reject_app_process(message):
    ...
    if app_id:
        ...
        if app:
            ...
            # СОХРАНЯЕМ ТЕКСТ АНКЕТЫ ПРИ ОТКЛОНЕНИИ!
            notifications = db.get_notifications("application", app_id)
            updated_text = (
            ...
            for notif_admin_id, notif_msg_id in notifications:
                try:
                    bot.edit_message_text(updated_text, notif_admin_id, notif_msg_id, parse_mode="HTML", reply_markup=None)
                except Exception as e:
            ...
            user_msg = f"❌ <b>Ваша анкета была отклонена</b>\n\n📝 <b>Причина:</b> {clean_html(reason)}"
            try: bot.send_message(app[1], user_msg, parse_mode="HTML")
            except Exception as e: logger.error(f" Ошибка отправки юзеру: {e}")

    bot.delete_state(admin_id, message.chat.id)

# ========== РАБОТА С ТИКЕТАМИ (ОГРАНИЧЕНИЕ ДЛЯ 1 УРОВНЯ) ==========
...
def ticket_reply_start(call):
    # ПРОВЕРКА УРОВНЯ: 1 УРОВЕНЬ НЕ МОЖЕТ ОТВЕЧАТЬ
    if db.get_admin_level(call.from_user.id) < 2:
        bot.answer_callback_query(call.id, "❌ У вас нет прав (нужен уровень 2+)", show_alert=True)
        return
...
def process_ticket_reply(message):
    ...
    if ticket:
        ...
        try:
            ...
            logger.error(f"Не удалось отправить ответ юзеру: {e}")
            
    bot.delete_state(admin_id, message.chat.id)

@bot.callback_query_handler(func=lambda call: call.data.startswith("ticket_close_"))
def close_ticket(call):
    # ПРОВЕРКА УРОВНЯ: 1 УРОВЕНЬ НЕ МОЖЕТ ЗАКРЫВАТЬ ТИКЕТЫ
    if db.get_admin_level(call.from_user.id) < 2:
        bot.answer_callback_query(call.id, "❌ У вас нет прав (нужен уровень 2+)", show_alert=True)
        return
    ...
    notifications = db.get_notifications("ticket", tid)
    updated_text = (
        f"✅ <b>Тикет #{tid} ЗАКРЫТ</b>\n"
        f"👑 Закрыл админ: @{clean_html(admin_uname)}"
    )
    ...
    db.delete_notifications("ticket", tid)
    bot.answer_callback_query(call.id, f"✅ Тикет #{tid} успешно закрыт.", show_alert=True)

# ========== РАЗДЕЛ АРХИВ (3+ УРОВЕНЬ) ==========
...
def admin_archive_menu(message):
    ...
    kb = types.InlineKeyboardMarkup()
    kb.row(
        types.InlineKeyboardButton("📝 Архив анкет", callback_data="arch_apps_p_0"),
        types.InlineKeyboardButton("🎫 Архив тикетов", callback_data="arch_tickets_p_0")
    )
    bot.send_message(message.chat.id, "🗄 **Раздел «Архив»**\nВыберите категорию для просмотра:", reply_markup=kb, parse_mode="Markdown")

# ----- АРХИВ АНКЕТ -----
@bot.callback_query_handler(func=lambda call: call.data.startswith("arch_apps_p_"))
def archive_apps_list(call):
    if db.get_admin_level(call.from_user.id) < 3:
        bot.answer_callback_query(call.id, "❌ Нет доступа", show_alert=True)
        return
    ...
    limit = 5
    offset = page * limit
    apps, total = db.get_archived_applications(limit=limit, offset=offset)

    if not apps:
        bot.edit_message_text("📭 Архив анкет пуст.", call.message.chat.id, call.message.message_id)
        bot.answer_callback_query(call.id)
        return

    text = f"🗄 <b>АРХИВ АНКЕТ (Страница {page + 1}):</b>\n\n"
    kb = types.InlineKeyboardMarkup()

    for app_id, user_id, username, status, created_at, reviewed_by in apps:
        status_icon = "⏳" if status == 'pending' else ("✅" if status == 'approved' else "❌")
        admin_str = f"| Админ: @{clean_html(db.get_username(reviewed_by))}" if reviewed_by else ""
        text += f"{status_icon} <b>Анкета #{app_id}</b> | @{clean_html(username)} {admin_str}\n"
        kb.add(types.InlineKeyboardButton(f"🔍 Открыть анкету #{app_id}", callback_data=f"arch_app_view_{app_id}_{page}"))

    nav_btns = []
    if page > 0:
        nav_btns.append(types.InlineKeyboardButton("◀️ Назад", callback_data=f"arch_apps_p_{page - 1}"))
    if (page + 1) * limit < total:
        nav_btns.append(types.InlineKeyboardButton("Вперёд ▶️", callback_data=f"arch_apps_p_{page + 1}"))
    
    if nav_btns: kb.row(*nav_btns)
    kb.row(types.InlineKeyboardButton("🔙 Назад в меню архива", callback_data="back_to_archive_main"))

    bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="HTML")
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda call: call.data.startswith("arch_app_view_"))
def archive_app_view(call):
    ...
    app = db.get_application(app_id)
    if app:
        # id, user_id, username, text, status, created_at, reviewed_at, review_reason, reviewed_by
        status_text = "⏳ На рассмотрении" if app[4] == 'pending' else ("✅ Одобрено" if app[4] == 'approved' else "❌ Отклонено")
        admin_uname = db.get_username(app[8]) if app[8] else "Не указан"
        reason_text = f"\n<b>Причина отказа:</b> {clean_html(app[7])}" if app[7] else ""
        
        text = (
            f"📝 <b>АНКЕТА #{app[0]}</b>\n\n"
            f"👤 <b>Пользователь:</b> @{clean_html(app[2])} (ID: <code>{app[1]}</code>)\n"
            f"📊 <b>Статус:</b> {status_text}\n"
            f"👑 <b>Проверил админ:</b> @{clean_html(admin_uname)}\n"
            f"📅 <b>Дата подачи:</b> {clean_html(app[5])[:16]}\n"
            f"{reason_text}\n\n"
            f"📋 <b>Текст анкеты:</b>\n{clean_html(app[3])}"
        )
        
        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton("🔙 К списку анкет", callback_data=f"arch_apps_p_{page}"))
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="HTML")
    bot.answer_callback_query(call.id)

# ----- АРХИВ ТИКЕТОВ -----
@bot.callback_query_handler(func=lambda call: call.data.startswith("arch_tickets_p_"))
def archive_tickets_list(call):
    if db.get_admin_level(call.from_user.id) < 3:
        bot.answer_callback_query(call.id, "❌ Нет доступа", show_alert=True)
        return
    ...
    limit = 5
    offset = page * limit
    tickets, total = db.get_archived_tickets(limit=limit, offset=offset)

    if not tickets:
        bot.edit_message_text("📭 Архив тикетов пуст.", call.message.chat.id, call.message.message_id)
        bot.answer_callback_query(call.id)
        return

    text = f"🗄 <b>АРХИВ ТИКЕТОВ (Страница {page + 1}):</b>\n\n"
    kb = types.InlineKeyboardMarkup()

    for tid, user_id, status, created_at, closed_by in tickets:
        status_icon = "🟢" if status == 'open' else "🔴"
        status_str = "Рассматривается" if status == 'open' else "Закрыто"
        admin_str = f"| Закрыл: @{clean_html(db.get_username(closed_by))}" if closed_by else ""
        
        text += f"{status_icon} <b>Тикет #{tid}</b> | @{clean_html(db.get_username(user_id))} ({status_str}) {admin_str}\n"
        kb.add(types.InlineKeyboardButton(f"📜 Чат тикета #{tid}", callback_data=f"arch_ticket_view_{tid}_{page}"))

    nav_btns = []
    if page > 0:
        nav_btns.append(types.InlineKeyboardButton("◀️ Назад", callback_data=f"arch_tickets_p_{page - 1}"))
    if (page + 1) * limit < total:
        nav_btns.append(types.InlineKeyboardButton("Вперёд ▶️", callback_data=f"arch_tickets_p_{page + 1}"))
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("arch_ticket_view_"))
def archive_ticket_view(call):
    parts = call.data.split("_")
    tid = int(parts[3])
    page = int(parts[4])
    ...
    if ticket:
        messages = db.get_ticket_messages(tid)
        status_str = "🟢 Открыт" if ticket[2] == 'open' else "🔴 Закрыт"
        closed_by_str = f"\n<b>Закрыл админ:</b> @{clean_html(db.get_username(ticket[4]))}" if ticket[4] else ""
        
        text = f"📋 <b>АРХИВ ТИКЕТА #{tid}</b>\n👤 <b>Пользователь:</b> @{clean_html(db.get_username(ticket[1]))}\n📊 <b>Статус:</b> {status_str}{closed_by_str}\n\n💬 <b>История сообщений:</b>\n\n"
        for msg in messages:
            author_id, is_admin = msg[1], msg[3]
            author_uname = db.get_username(author_id)
            role = f"👑 Админ (@{clean_html(author_uname)})" if is_admin else f"👤 @{clean_html(author_uname)}"
        ...
        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton("🔙 К списку тикетов", callback_data=f"arch_tickets_p_{page}"))
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="HTML")
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda call: call.data == "back_to_archive_main")
def back_to_archive_main(call):
    kb = types.InlineKeyboardMarkup()
    kb.row(
        types.InlineKeyboardButton("📝 Архив анкет", callback_data="arch_apps_p_0"),
        types.InlineKeyboardButton("🎫 Архив тикетов", callback_data="arch_tickets_p_0")
    )
    bot.edit_message_text("🗄 **Раздел «Архив»**\nВыберите категорию для просмотра:", call.message.chat.id, call.message.message_id, reply_markup=kb, parse_mode="Markdown")
    bot.answer_callback_query(call.id)
